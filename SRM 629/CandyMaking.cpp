...
public:
	double findSuitableDensity(vector <int> containerVolume, vector <int> desiredWeight) {
		double res = (1000000000)*1.0;
		double nom, denom, tmp, lres;
		int k = containerVolume.size();
		for (int i=0; i<k; ++i)
		{
		    lres = 0.0;
            denom = desiredWeight[i];
            nom = containerVolume[i];
            for (int j=0; j<k; ++j)
            {
                tmp = ((denom*containerVolume[j])/nom);
                lres += abs(tmp - desiredWeight[j]);
            }
            res = min(res,lres);
		}
		return res;
